...
def predict_pdf(path) :
        ...
        try :
                ...
                for i in rawList1:
                    if re.search(x, i, re.IGNORECASE):
                        ...
                        if i.startswith('M') :
                            ...
                            result_gender = list(i.split(':'))[1].strip()
                            if result_gender.startswith('M') :
                               dictionary['Gender'] = 1
                               continue

                            elif result_gender.startswith('F') :
                                dictionary['Gender'] = 0
                                continue
            ...
            print(custom_values_list)

            model_acute_l = joblib.load('DocTork-master/ML_Model/API_2/PDF/Leukemia/Model/RandomForestModel(AcuteL-m,f).h5')
            model_cml = joblib.load('DocTork-master/ML_Model/API_2/PDF/Leukemia/Model/RandomForestModel(CML-m,f).h5')
            ...
            custom_data = pd.DataFrame(data = np.array([custom_values_list]) , columns = features[:-1])
